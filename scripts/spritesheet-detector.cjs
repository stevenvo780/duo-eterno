...
class SpritesheetDetector {
    ...
    async processDirectory(dirPath) {
        ...
        for (const file of pngFiles) {
            ...
            if (dimensions.width < CONFIG.minSpritesheetSize.width || 
            ...
            if (detection.isSpritesheet) {
                ...
                console.log(`   └─ ${detection.layout.columns}x${detection.layout.rows} frames (${detection.layout.frameWidth}x${detection.layout.frameHeight}px)`);
                console.log(`   └─ Tipo: ${detection.animationType} | Confianza: ${detection.layout.confidence}%`);
