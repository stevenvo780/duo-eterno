...
class TelemetryCollector {
  ...
  startRecording(): void {
    ...
    this.snapshots = [];
    console.log('🔍 Telemetría iniciada - Auto-guardado cada 30 segundos');
  ...
  stopRecording(): void {
    ...
    // Guardar datos finales al detener
    this.autoSaveData();
  ...
  private autoSaveData(): void {
    ...
    try {
      ...
      const reports = JSON.parse(localStorage.getItem('telemetry-reports') || '[]');
      ...
      localStorage.setItem('telemetry-reports', JSON.stringify(reports));
      localStorage.setItem('latest-telemetry-report', JSON.stringify(reportData));
      
      console.log('📊 Reporte de telemetría guardado:', {
        snapshots: this.snapshots.length,
        problems: analysis.problems.length,
        systemStability: analysis.systemAnalysis.systemStability,
        avgSurvival: Math.round(analysis.systemAnalysis.averageSurvivalTime)
      });
    } catch (error) {
  ...
  private generateSummary(analysis: TelemetryAnalysis): {
    score: number;
    status: string;
    ...
    const criticalProblems = analysis.problems.filter(p => p.severity === 'critical').length;
    const highProblems = analysis.problems.filter(p => p.severity === 'high').length;
    const totalEntities = analysis.entityAnalysis.length;
    const deadEntities = analysis.entityAnalysis.filter(e => e.deathCause).length;
    
    return {
      score: this.calculateSystemScore(analysis),
      status: analysis.systemAnalysis.systemStability,
      criticalIssues: criticalProblems,
      highIssues: highProblems,
      survivalRate: totalEntities > 0 ? ((totalEntities - deadEntities) / totalEntities * 100) : 0,
      avgSurvivalTime: Math.round(analysis.systemAnalysis.averageSurvivalTime),
      topRecommendations: analysis.recommendations.slice(0, 3)
    };
  }

  private calculateSystemScore(analysis: TelemetryAnalysis): number {
    let score = 100;
    ...
    // Penalizar por inestabilidad del sistema
    if (analysis.systemAnalysis.systemStability === 'declining') score -= 30;
    else if (analysis.systemAnalysis.systemStability === 'unstable') score -= 15;
    
    // Bonus por tiempo de supervivencia alto
    if (analysis.systemAnalysis.averageSurvivalTime > 100) score += 10;
