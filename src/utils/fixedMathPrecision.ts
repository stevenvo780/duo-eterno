...
export const areEqual = (
  a: number,
  b: number,
  epsilon: number = MATH.HIGH_PRECISION_EPSILON
): boolean => {
  if (a === b) return true;
 ...
 */
export const balancedNoise = (
  x: number,
  y: number,
  variabilityFactor: number = 0.3,
  seed: number = 54321
): number => {
...
export const validateNumber = (
  ...
  const shouldLog =
    !silent && (process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'test');
