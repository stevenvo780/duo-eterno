...
export function getEntityAnimations(entityName: string): AnimationAsset[] {
  return getAllAnimations().filter(
    anim => anim.id.includes(entityName) || anim.name.includes(entityName)
  );
}
...
export function searchAssets(query: string): (AnimationAsset | StaticSpriteAsset)[] {
  ...
  function searchInFolder(folder: AssetFolder) {
    ...
    results.push(
      ...folder.animations.filter(
        anim =>
          anim.id.toLowerCase().includes(searchTerm) || anim.name.toLowerCase().includes(searchTerm)
      )
    );

    // Buscar en sprites estáticos
    results.push(
      ...folder.staticSprites.filter(
        sprite =>
