...
const DebugPanel: React.FC<DebugPanelProps> = ({ visible, onClose }) => {
  ...
  return (
    ...
    >
      ...
      >
        <h3 style={{ margin: 0, fontSize: '16px', fontWeight: '600' }}>🔍 Dinámicas Debug</h3>
        <button
          onClick={onClose}
          style={{
            background: 'none',
            ...
            .map((log, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ color: '#e5e7eb', lineHeight: '1.3' }}>{log.message}</div>
                  {log.data ? (
      ...
      >
        {logs.length} logs mostrados • Usa console.log(dynamicsLogger.generateReport()) para más
        detalles
      </div>
    </div>
  );
